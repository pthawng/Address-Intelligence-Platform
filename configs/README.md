...
Nguồn triển khai: `internal/platform/config/config.go`. Cả API, indexer và worker gọi `config.Load()` trước khi khởi tạo runtime. Thứ tự áp dụng: biến chính có giá trị → alias có giá trị (nếu có) → mặc định. Biến rỗng được xem như chưa thiết lập; giá trị có khoảng trắng không tự động được trim.
...
| `DATABASE_URL` | rỗng | URL `postgres://` hoặc `postgresql://`, có host, port hợp lệ nếu khai báo; bắt buộc ở staging/production |
| `ELASTICSEARCH_URL` | `http://localhost:9200` | URL HTTP/HTTPS, có host, port hợp lệ nếu khai báo |
...
Load thất bại trả về lỗi ghi tên biến, không đưa giá trị thô vào thông báo lỗi; entry point thoát với mã 1. Không log toàn bộ `Config` vì connection string có thể chứa secret. Tất cả runtime hiện validate cùng bộ cấu hình, kể cả biến HTTP ở worker/indexer.
...
Ở development/test, database có thể để trống để chạy skeleton. Hiện chưa có adapter kết nối PostgreSQL/Elasticsearch, vòng lặp xử lý outbox hay telemetry exporter. `/health/ready` hiện trả cùng kết quả tĩnh với `/health/live`, chưa kiểm tra dependency.
...
Compose đặt `DATABASE_URL` từ `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`, và đặt `ELASTICSEARCH_URL=http://elasticsearch:9200`. API luôn lắng nghe `HTTP_ADDRESS=:8080` bên trong container; `API_PORT` chỉ đổi port công bố trên máy host. Các giá trị trong `environment` của Compose ưu tiên hơn `env_file`.
